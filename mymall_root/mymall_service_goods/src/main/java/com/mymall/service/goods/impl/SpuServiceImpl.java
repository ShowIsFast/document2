...
import com.mymall.service.goods.dao.CategoryBrandMapper;
import com.mymall.service.goods.dao.CategoryMapper;
import com.mymall.service.goods.dao.SkuMapper;
import com.mymall.service.goods.dao.SpuMapper;
import com.mymall.pojo.entity.PageResult;
import com.mymall.pojo.goods.*;
...
public class SpuServiceImpl implements SpuService {
    ...
    private static Logger log = LoggerFactory.getLogger(SpuServiceImpl.class);
    ...
    public Map<String,Object> selectNewByOne() {
        return spuMapper.selectNewByOne();
    }
     ...
     * 1、 保存 SKU List
     * 2、 将商品价格进行缓存
     * 3、 发送消息到 MQ，异步为每一个 SKU 生成静态 html 页面
     * 4、 [同步将 SKU 写入到 ES] 进行 优化，调整为：通过MQ异步进行
     * 5、 保存 SPU
     * @param goods 商品组合实体类
     */
    public void saveGoods(Goods goods) {
        log.info("[mymall_wservice_goods][SpuServiiceImpl][saveGoods][request param][goods:]");
        ...
        for (Sku sku :skuList){
            ...
            log.info("[mymall_service_goods][SpuServiceImpl][saveGoods][sku.getSpec():]");
            log.info(sku.getSpec());
            ...
            sku.setSn(spu.getSn());//商品货号
            skuMapper.insert(sku);//插入sku表数据
            String i = sku.getId();
            specItems+=sku.getSpec()+",";
            //重新将价格更新到缓存
            ...
            try {
                ...
                map.put("sku_id",String.valueOf(i));
                map.put("spu_id",spu.getId());
                log.info("添加商品对应的消息：{}",JSON.toJSONString(map));
                rocketMQTemplate.syncSend("goods-es-sms",JSON.toJSONString(map));
            } catch (Exception e) {
                log.error("rocketmq发送添加商品对应的消息失败",e);
            }
        }
        if(!specItems.equals("")){
            specItems=specItems.substring(0,specItems.length()-1);
        }
        ...
        System.out.println("==spu.getBrandId()=="+spu.getBrandId());
        if(null != spu.getBrandId() && !"null".equals(spu.getBrandId())){
            //添加 模板品牌关联
            //先查询是否存在记录
