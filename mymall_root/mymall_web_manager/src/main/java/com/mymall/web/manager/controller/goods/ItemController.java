...
public class ItemController {
    ...
    @GetMapping("/testGoodsAll")
    public void testGoodsAll(String page,String pageSize){
        PageResult<Spu> page1 = spuService.findPage(Integer.valueOf(page), Integer.valueOf(pageSize));
        List<Spu> rows = page1.getRows();
        System.out.println("rows.size:"+rows.size());

        for (int i =0;i< rows.size() ; i++){
            System.out.println(i+"===="+rows.get(i).toString());
            createPage(rows.get(i).getId());
        }
    }
